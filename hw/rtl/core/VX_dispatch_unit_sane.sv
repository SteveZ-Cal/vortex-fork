...
module VX_dispatch_unit_sane import VX_gpu_pkg::*; #(    
    ...
    for (genvar i = 0; i < `ISSUE_WIDTH; ++i) begin : genblk2
        assign dispatch_valid[i] = dispatch_if[i].valid;
        assign dispatch_data[i] = {dispatch_if[i].data,2'b0};
    ...
    for (genvar i = 0; i < BATCH_COUNT; ++i) begin : genblk1
    ...
    for (genvar block_idx = 0; block_idx < BLOCK_SIZE; ++block_idx) begin : genblk2
        ...
        if (`NUM_THREADS != NUM_LANES) begin : genblk3
            ...
            for (genvar i = 0; i < NUM_PACKETS; ++i) begin : genblk4
                for (genvar j = 0; j < NUM_LANES; ++j) begin : genblk5
            ...
            for (genvar i = 0; i < NUM_PACKETS; ++i) begin : genblk6                
        ...
        if (BATCH_COUNT != 1) begin : genblk10
            if (BLOCK_SIZE != 1) begin : genblk10
                assign isw = {batch_idx, BLOCK_SIZE_W'(block_idx)};
            end else begin : genblk11
                assign isw = batch_idx;
            end
        end else begin : genblk12
