...
module VX_pe_switch import VX_gpu_pkg::*; #(
    ...
    //wire [6373:0] data_in_new;

    //assign data_in_new = 0;
    ...
    for (genvar i = 0; i < PE_COUNT; ++i) begin : g_execute_out_if
        ...
        assign execute_out_if[i].data = {pe_req_data[i],68'b0};
    ...
    wire [2199:0] commit_in_if_data_new;
    
    for (genvar i = 0; i < PE_COUNT; ++i) begin : g_commit_in_if
        assign pe_rsp_valid[i] = commit_in_if[i].valid;
        assign pe_rsp_data[i] = commit_in_if_data_new ; //commit_in_if[i].data;
