...
static void matmul_cpu(TYPE* C, const TYPE* A, const TYPE* B, uint32_t M, uint32_t N, uint32_t K) {
...
int main(int argc, char *argv[]) {
  ...
  // tile format ratio
  uint32_t o_ratio = XlenB / sizeof(TYPE);
  uint32_t i_ratio = XlenB / sizeof(TYPE);
  ...
  kernel_arg.block_dim[0] = NT; // warp size
  kernel_arg.block_dim[1] = 1;
  if (kernel_arg.grid_dim[0]  != 1
   || kernel_arg.grid_dim[1]  != 1
   || kernel_arg.block_dim[0] != 32
   || kernel_arg.block_dim[1] != 1)
  {
    std::cout << "Error: Dimensions must be 16x16x16\n";
    return -1;
  }

  // must be 32 threads exactly
  if (NT != 32) {
    std::cout << "Error: Number of threads must be 32\n";
    return -1;
  }
