...
int main(int argc, char *argv[]) {
  ...
  std::cout << "GPU tensor logNT=" << logNT << ", o_ratio=" << o_ratio << ", i_ratio=" << i_ratio << ", XlenB=" << XlenB << std::endl;
  std::cout << "GPU tensor tileM=" << tileM << ", tileN=" << tileN << ", tileK=" << tileK << std::endl;
  std::cout << "GPU tensor M=" << M << ", N=" << N << ", K=" << K << std::endl;


  if ((M % tileM) != 0) {
    std::cout << "Error: M must be a multiple of tensor tileM!" << " M = " << M << " , tileM = " << tileM <<  std::endl;
    return -1;
  }

  if ((N % tileN) != 0) {
    std::cout << "Error: N must be a multiple of tensor tileN!" << " N = " << N << " , tileN = " << tileN << std::endl;
    return -1;
  }

  if ((K % tileK) != 0) {
    std::cout << "Error: K must be a multiple of tensor tileK!" << " K = " << K << " , tileK = " << tileK << std::endl;
    return -1;
  }
